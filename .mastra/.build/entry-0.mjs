...
const UCR_OFFENSE_DESCRIPTIONS = {
...
const searchAgenciesTool = createTool({
  id: "search-agencies",
  description: "Search for law enforcement agency metadata (names, ORIs, locations). Use this ONLY to find agency ORI identifiers for a specific city/county - NOT for counting agencies or aggregating by state.",
  ...
  execute: async (input) => {
    ...
    const limit = Math.min(input.limit || 100, 1e4);
    const sql = `
      SELECT
        ori,
        agency_name,
...
const getIncidentCountsTool = createTool({
  id: "get-incident-counts",
  description: "Get crime incident counts, rates, and comparisons from NIBRS data. Use for comparing crime across agencies/cities, getting homicide counts by agency, comparing crime rates between states, and ranking agencies by crime type.",
  inputSchema: z.object({
    ...
    limit: z.number().nullish().default(1e3).describe("Maximum results (max 10000)")
  }),
  outputSchema: z.object({
  ...
  execute: async (input) => {
    ...
    const whereClause = buildWhereClause(conditions);
    const limit = Math.min(input.limit || 1e3, 1e4);
    let groupByClause;
    let selectClause;
    let orderByClause;
...
const getOffenseSummaryTool = createTool({
  id: "get-offense-summary",
  description: "Get detailed offense statistics from NIBRS data. Can analyze offenses by type, location, weapon used, or bias motivation.",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
...
const getVictimDemographicsTool = createTool({
  id: "get-victim-demographics",
  description: "Get victim demographic breakdowns (sex, race, age) from NIBRS data. Use this to analyze WHO the victims are.",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
    ori: z.string().nullish().describe("Filter by agency ORI"),
    fromYear: z.number().int().nullish().describe("Start year"),
  ...
  execute: async (input) => {
    ...
    const whereClause = buildWhereClause(conditions);
    const limit = Math.min(input.limit || 100, 1e3);
    let selectClause;
    let groupByClause;
    switch (input.groupBy) {
      case "race":
...
const getCrimeTrendsTool = createTool({
  id: "get-crime-trends",
  description: "Get crime trend data over time. Returns time-series data showing incident counts by year or month.",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
    ori: z.string().nullish().describe("Filter by agency ORI"),
    offenseCode: UCROffenseCodeEnum.nullish().describe("Filter by offense code"),
  ...
  execute: async (input) => {
    ...
    const rows = await runQuery(sql);
    const totalCount = rows.reduce((sum, row) => sum + (Number(row.count) || 0), 0);
    const title = input.offenseCode ? `${UCR_OFFENSE_DESCRIPTIONS[input.offenseCode] || input.offenseCode} Trends` : "Crime Trends";
    return {
      title,
      data: rows.map((row) => ({
...
const getWeaponAnalysisTool = createTool({
  id: "get-weapon-analysis",
  description: "Analyze weapon usage in crimes from NIBRS data. Shows what types of weapons are used in offenses.",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
    ori: z.string().nullish().describe("Filter by agency ORI"),
    fromYear: z.number().int().nullish().describe("Start year"),
  ...
  execute: async (input) => {
    const conditions = ["o.type_weapon_force_involved1 IS NOT NULL"];
    if (input.stateAbbr) {
      conditions.push(`ag.state_abbr = '${input.stateAbbr}'`);
    }
    if (input.ori) {
    ...
    const whereClause = buildWhereClause(conditions);
    const limit = Math.min(input.limit || 50, 1e4);
    let selectClause;
    let groupByClause;
    let orderByClause;
    switch (input.groupBy) {
    ...
    const rows = await runQuery(sql);
    rows.forEach((row) => {
      if (row.weapon_code !== null && row.weapon_code !== void 0) {
...
const getBiasAnalysisTool = createTool({
  id: "get-bias-analysis",
  description: "Analyze hate crime bias motivations from NIBRS data. Shows the distribution of bias motivations (racial, religious, sexual orientation, etc.) in crimes.",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
    ori: z.string().nullish().describe("Filter by agency ORI"),
    fromYear: z.number().int().nullish().describe("Start year"),
  ...
  execute: async (input) => {
    const conditions = [
      ...
      "o.bias_motivation != '88'"
      // Exclude "None (no bias)"
    ];
    if (input.stateAbbr) {
      conditions.push(`ag.state_abbr = '${input.stateAbbr}'`);
    }
...
const getLocationAnalysisTool = createTool({
  id: "get-location-analysis",
  description: "Analyze where crimes occur by location type from NIBRS data. Shows distribution of offenses across different location types (residence, street, bar, etc.).",
  inputSchema: z.object({
    stateAbbr: StateAbbrEnum.nullish().describe("Filter by state"),
    ori: z.string().nullish().describe("Filter by agency ORI"),
    fromYear: z.number().int().nullish().describe("Start year"),
  ...
  execute: async (input) => {
    const conditions = ["o.location_type IS NOT NULL"];
    if (input.stateAbbr) {
      conditions.push(`ag.state_abbr = '${input.stateAbbr}'`);
    }
    if (input.ori) {
...
const executeCustomQueryTool = createTool({
...
**HATE CRIMES:** WHERE bias_motivation != '88' (exclude 'None')`,
  inputSchema: z.object({
    ...
    limit: z.number().nullish().default(1e3).describe("Maximum rows to return (capped at 10000)")
  }),
  outputSchema: z.object({
    results: z.array(z.record(z.string(), z.unknown())),
    row_count: z.number(),
  ...
  execute: async (input) => {
    ...
    const maxLimit = Math.min(input.limit || 1e3, 1e4);
...
"use strict";
const NIBRS_SYSTEM_PROMPT = `You are an intelligent data analyst with COMPLETE access to the FBI NIBRS (National Incident-Based Reporting System) crime database via BigQuery.
...
**\u{1F534} CRITICAL: DEFAULT TIME PERIOD**
...
_"\u{1F4C5} **Note:** This analysis uses 2025 data (most recent complete year). Would you like to see:_
...
**\u{1F534} CRITICAL: EXECUTION BEHAVIOR**
...
**\u{1F534} CRITICAL: FINAL RESPONSE FORMAT**
...
**\u{1F6AB} NEVER MENTION THESE IN YOUR FINAL RESPONSE:**
